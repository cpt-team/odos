    ...
    private fun generateAchievementList(): List<Achievement> {
        ...
        if (odosCnt != null && diaryCnt != null) {
            if((odosCnt.toInt()+diaryCnt.toInt()) >= 7){
                achievementList.add(Achievement("7일동안 나를 기록해요", "다이어리, odos 7개 작성", R.drawable.ic_achieve,true,R.drawable.field4))
            }
            else if((odosCnt.toInt()+diaryCnt.toInt()) < 7){
                achievementList.add(Achievement("7일동안 나를 기록해요", "다이어리, odos 7개 작성", R.drawable.ic_achieve,false,R.drawable.field4))
            }
        }

        if (emotionList != null) {
            if(emotionList.size == 5){
                achievementList.add(Achievement("내 감정에 솔직하기", "odos의 emotion 다 써보기", R.drawable.ic_achieve,true,R.drawable.pinkpod))
            }
            else if(emotionList.size < 5){
                achievementList.add(Achievement("내 감정에 솔직하기", "odos의 emotion 다 써보기", R.drawable.ic_achieve,false,R.drawable.smilepod))
            }
        }
        if (whetherList != null) {
            if(whetherList.size == 5){
                achievementList.add(Achievement("너의 하루는?", "whether 다 써보기", R.drawable.ic_achieve,true,R.drawable.snow4))
            }
            else if(whetherList.size < 5){
                achievementList.add(Achievement("너의 하루는?", "whether 다 써보기", R.drawable.ic_achieve,false,R.drawable.snow3))
            }
        }
        if (odosCnt != null && diaryCnt != null) {
            if((odosCnt.toInt()+diaryCnt.toInt()) >= 100){
                achievementList.add(Achievement("기록 첫 기념일!!", "총 100일 작성", R.drawable.ic_achieve,true,R.drawable.yellowpod))
            }
